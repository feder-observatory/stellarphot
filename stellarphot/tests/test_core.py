...
photdata = np.array([[1, 2049.145245206124, 2054.0849947477964, 109070.60831212997, 154443.9371254444, 78.17278712191924,
...
photcolnames = ['id','xcenter','ycenter','aperture_sum','annulus_sum','ra','dec','sky_per_pix_avg',
        'sky_per_pix_med','sky_per_pix_std','fwhm_x','fwhm_y','width','aperture','aperture_area',
        'annulus_inner','annulus_outer','annulus_area','exposure','date-obs','night','aperture_net_cnts',
        'bjd','mag_inst','airmass','passband','file','star_id','mag_error','noise','noise-aij','snr']
photcoltypes = ['int', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float', 'float',
        'float', 'float', 'float', 'float', 'float', 'float', 'float', 'str', 'int', 'float', 'float', 'float',
        'float', 'str', 'str', 'int', 'float', 'float', 'float', 'float']
photcolunits = [None, u.pix, u.pix, u.adu, None, u.deg, u.deg, u.adu, u.adu, u.adu, None, None, None,
...
testphot_data = Table(photdata, names=photcolnames, dtype=photcoltypes, units=photcolunits)

# Convert times to correct time format but leave bad units
testphot_goodTime = testphot_data.copy()
testphot_goodTime['date-obs'] = Column(data=Time(testphot_goodTime['date-obs'], format='isot', scale='utc'), name='date-obs')
...
def test_photometry_badtime():
    with pytest.raises(ValueError):
        phot_data = PhotometryData(observatory=feder_obs, camera=feder_cg_16m, passband_map=feder_passbands, data=testphot_data)

def test_photometry_inconsistent_count_units():
    with pytest.raises(ValueError):
        phot_data = PhotometryData(observatory=feder_obs, camera=feder_cg_16m, passband_map=feder_passbands, data=testphot_goodTime)

def test_photometry_inconsistent_badunits():
    with pytest.raises(ValueError):
        phot_data = PhotometryData(observatory=feder_obs, camera=feder_cg_16m, passband_map=feder_passbands, data=testphot_goodCounts)

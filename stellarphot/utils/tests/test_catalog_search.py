...
def a_table(masked=False):
    test_table = Table([(1, 2, 3), (1, -1, -1)], names=("a", "b"), masked=masked)
    return test_table
...
@pytest.mark.parametrize("condition", [">0", "=1", "!=-1", ">=1"])
...
def test_in_frame():
    ...
    coordinates_all_in = SkyCoord(
        ra=[ra_cen, ra_cen + 1], dec=[dec_cen, dec_cen + 1], unit="degree"
    )
    should_be_all_in = in_frame(wcs, coordinates_all_in)
    assert all(should_be_all_in)

    some_in_some_out = SkyCoord(
        ra=[50, ra_cen + 1], dec=[0, dec_cen + 1], unit="degree"
    )
...
def test_catalog_search(clip, data_file):
    ...
    actual = catalog_search(wcs, CCD_SHAPE, "B/vsx/vsx", clip_by_frame=clip)
...
def test_catalog_search_from_wcs_or_coord():
    ...
    vsx_vars = catalog_search(wcs, [4096, 4096], "B/vsx/vsx", clip_by_frame=False)
    ...
    # so don't do that
    vsx_vars2 = catalog_search(
        cen_coord, [4096, 4096], "B/vsx/vsx", clip_by_frame=False
    )
...
def test_catalog_search_with_coord_and_frame_clip_fails():
    ...
    cen_coord = wcs.pixel_to_world(4096 / 2, 4096 / 2)
    with pytest.raises(ValueError) as e:
        _ = catalog_search(cen_coord, [4096, 4096], "B/vsx/vsx", clip_by_frame=True)
...
def test_find_apass():
    ...
    expected_low_error = Table.read(
        get_pkg_data_filename("data/low_error_apass_ey_uma_sorted_ra_first_20.fits")
    )
    wcs_file = get_pkg_data_filename("data/sample_wcs_ey_uma.fits")
    with fits.open(wcs_file) as hdulist:
        with warnings.catch_warnings():

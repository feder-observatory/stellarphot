...
| `transform_to_catalog()` | `stellarphot.utils` | photometry table, passband options, `vary` (terms to fit), `expected` (ranges to check them against) | Cross-match to APASS DR9 / RefCat2 and fit `calibrated_from_instrumental()` |
